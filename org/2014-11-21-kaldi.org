...
**** TODO 使用kaldi搭建拨号系统
**** TODO SGMM
**** DONE kaldi & htk 在训练上的相同点&不同点&对应
**** DONE 决策树在哪儿，viterbi对齐
**** DONE 训练中LDA & MLLT & MMI是必须的吗
**** DONE fst的具体使用方式
**** DONE wfst已经生成，如何解码，和htk的区别
**** DONE for (fst::ArcIterator<fst::Fst<Arc> > aiter(fst_, state);
          !aiter.Done();
          aiter.Next())
**** DONE std min max swap ...
**** TODO token gc引用计数原理
**** DONE decodable->LogLikelihood(frame, arc.ilabel)
**** DONE Tree-based State Tying for High Accuracy Acoustic Modeling
**** unsorted_map
**** DONE 对齐
**** TODO 解码时有类似wdnet的东西限制搜索空间吗？
**** DONE 如何加高斯，gmm split & merge, htk中是怎么增加高斯的
**** split
id = arg max w_{k}

w_{n}=w_{max}/2, w_{n}为新加入高斯权值

u_{n} = u_{id}, sigma_{n}= sigma_{id}

why?

且训练时每个模型的高斯数不一定相等，有意思哈！

**** DONE Kaldi中的并行JOB utils/run.pl
***** 主要特点
1. 错误日志
2. 计算时间
***** 示例
